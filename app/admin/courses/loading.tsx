  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-xl sm:text-2xl font-bold text-default-900">จัดการรายวิชา</h1>
          <p className="text-sm text-default-500">จัดการรายวิชาและกลุ่มเรียน</p>
        </div>
      ...
      <Card className="border border-default-200 shadow-sm">
        <CardBody className="p-0">
          ...
          <div className="flex items-center gap-4 px-4 py-3 border-b border-default-100 bg-default-50">
            {[...Array(7)].map((_, i) => (
              <Skeleton key={i} className={`h-4 rounded-lg ${i === 0 ? "w-24" : i === 1 ? "w-40" : i === 2 ? "w-20" : "w-24"}`} />
            ))}
          </div>
